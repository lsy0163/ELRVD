...
from models import RViDeNet, RViDeNet_
from utils import *
...
parser.add_argument('--num_epochs', dest='num_epochs', type=int, default=500, help='num_epochs')
...
save_dir = "/database/iyj0121/dataset/4k_vnt/ckpt/finetune/finetune_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg_scene_from_1_to_6_video_debug" #_model_size_up
...
log_dir = "/database/iyj0121/tensorboard/finetune_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg_scene_from_1_to_6_video/" #_model_size_up
...
predenoiser = torch.load("/database/iyj0121/dataset/4k_vnt/ckpt/predenoising/predenoising_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg/model_epoch700.pth") #"/database/iyj0121/dataset/Sony/predenoising_all_ISO/model_epoch700.pth"
...
if initial_epoch > 0:
    ...
    denoiser = torch.load(os.path.join("/database/iyj0121/dataset/4k_vnt/ckpt/pretrain/pretrain_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg/model_epoch33.pth")).cuda() #original code
    #denoiser = torch.load(os.path.join("/database/iyj0121/ckpt/jione/rvidenet/pretrain_sfb2/model_epoch33.pth")).cuda()
    #denoiser = torch.load(os.path.join("/database/iyj0121/ckpt/rvidnet/finetuning_CRVD_rggb_baseline/model_epoch300.pth")).cuda()
    #denoiser = torch.load("/database/iyj0121/ckpt/rvidenet_finetuning/finetune_noise_estimation_crvd_method_neif_on_4k_calibration_gain_only_29p4_hcg/model_epoch70.pth").cuda()
...
#opt = optim.Adam([{'params': base_params}, {'params': denoiser.recon_trunk.parameters(), 'lr': 1e-7}, {'params': denoiser.cbam.parameters(), 'lr': 1e-7}, {'params': denoiser.conv_last.parameters(), 'lr': 1e-7}], lr = 1e-6)
...
#opt = optim.Adam([{'params': base_params}, {'params': denoiser.recon_trunk.parameters(), 'lr': 1e-4}, {'params': denoiser.cbam.parameters(), 'lr': 1e-4}, {'params': denoiser.conv_last.parameters(), 'lr': 1e-4}], lr = 1e-5)
    ...
    for batch_id in range(int(train_data_length/batch_size)):
        ...
        while batch_num<batch_size:
            ...
            for shift in range(-1,2):
                ...
                gt_raw_path = f'/database/iyj0121/dataset/4k_vnt/VNT_241120_pair_video/scene{scene_ind}/scene{scene_ind}_frame{frame_ind + shift}_{gain_list[noisy_level-1]}db/cleaned_bayer_frame.raw'
                ...
                if shift == 0:
                    for self_consistency_index in range(4):
                        ...
                        noisy_raw_path = f'/database/iyj0121/dataset/4k_vnt/VNT_241120_pair_video/scene{scene_ind}/scene{scene_ind}_frame{frame_ind + shift}_{gain_list[noisy_level-1]}db/dump_bayer_frame_{(noisy_frame_index_for_current + self_consistency_index):05d}.raw'
                    ...
                    noisy_frame_index_for_other = np.random.randint(0, 240)
                    ...
                    noisy_raw_path = f'/database/iyj0121/dataset/4k_vnt/VNT_241120_pair_video/scene{scene_ind}/scene{scene_ind}_frame{frame_ind + shift}_{gain_list[noisy_level-1]}db/dump_bayer_frame_{noisy_frame_index_for_other:05d}.raw'
                    noisy_raw = np.fromfile(noisy_raw_path, dtype=np.uint16).reshape((2160, 3840))
        ...
        loss = raw_l1_loss + self_consistency_loss + 0.5*srgb_l1_loss #original code
        loss = loss.mean()
        loss.backward()
        opt.step()
