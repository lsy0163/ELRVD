...
save_dir = "/database/iyj0121/dataset/SRVD_data/pretrain_signal_independent_dependent_neif_noise_estimation_gain_only_29p4_FastDVDnet_no_predenoising_5frame/"
...
log_dir = "/database/iyj0121/tensorboard/pretrain_signal_independent_dependent_neif_noise_estimation_gain_only_29p4_FastDVDnet_no_predenoising_5frame/"
...
predenoiser = torch.load("/database/iyj0121/dataset/Sony/predenoising_signal_independent_dependent_neif_noise_estimation_gain_only_29p4_final/model_epoch700.pth")
    ...
    for batch_id in range(int(len(gt_paths)/batch_size)):
        ...
        while batch_num<batch_size:
            ...
            if 'MOT17-02_raw' in gt_path:
                if frame_id<4 or frame_id > len(gt_paths1)-4:
                    continue
            if 'MOT17-09_raw' in gt_path:
                if frame_id<4 or frame_id > len(gt_paths2)-4:
                    continue
            if 'MOT17-10_raw' in gt_path:
                if frame_id<4 or frame_id > len(gt_paths3)-4:
                    continue
            if 'MOT17-11_raw' in gt_path:           
                if frame_id<4 or frame_id > len(gt_paths4)-4:
                    continue
            batch_num += 1
