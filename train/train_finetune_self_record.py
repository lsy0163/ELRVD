...
from models import RViDeNet, RViDeNet_sfb, RViDeNet_ECBAM
from utils import *
...
parser.add_argument('--num_epochs', dest='num_epochs', type=int, default=600, help='num_epochs')
...
save_dir = "/database/iyj0121/ckpt/rvidenet_finetuning/finetune_noise_estimation_crvd_method_neif_gain_only_29p4_ecbam_texture_loss/"
...
log_dir = "/database/iyj0121/tensorboard/finetune_noise_estimation_crvd_method_neif_gain_only_29p4_ecbam_texture_loss/"
...
predenoiser = torch.load("/database/iyj0121/result/tcsvt/predenoising_noise_estimation_crvd_method_neif_gain_only_29p4/model_epoch700.pth") #"/database/iyj0121/dataset/Sony/predenoising_all_ISO/model_epoch700.pth"
...
if initial_epoch > 0:
    ...
    denoiser = torch.load(os.path.join("/database/iyj0121/result/tcsvt/pretrain_noise_estimation_crvd_method_neif_gain_only_29p4_ECBAM/model_epoch33.pth")).cuda()
...
recon_params2 = list(map(id, denoiser.ecbam.parameters()))
...
opt = optim.Adam([{'params': base_params}, {'params': denoiser.recon_trunk.parameters(), 'lr': 1e-5}, {'params': denoiser.ecbam.parameters(), 'lr': 1e-5}, {'params': denoiser.conv_last.parameters(), 'lr': 1e-5}], lr = 1e-6)
...
for epoch in range(initial_epoch, args.num_epochs+1):
    ...
    for batch_id in range(int(train_data_length/batch_size)):
        ...
        while batch_num<batch_size:
            ...
            frame_ind = np.random.randint(1,59+1)
            #noisy_level = np.random.randint(1,5+1)
            noisy_level = 2
            noisy_frame_index_for_current = np.random.randint(1,96+1)
