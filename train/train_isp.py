...
save_dir = "/database/iyj0121/dataset/Sony/isp_old/"
if not os.path.isdir(save_dir):
    os.makedirs(save_dir)

input_paths = glob.glob('/database/iyj0121/dataset/Sony/long_raw_arw/*.raw')
input_paths.sort()
gt_paths = glob.glob('/database/iyj0121/dataset/Sony/long_isp_png/*.png')
gt_paths.sort()

ps = args.patch_size  # patch size for training

log_dir = "/database/iyj0121/dataset/Sony/log/"
...
for epoch in range(initial_epoch, args.num_epochs+1):
    ...
    for ind in np.random.permutation(len(gt_paths)):
        ...
        if input_raws[scene_id] is None:
            ...
            try:
                input_raw = rawpy.imread(input_paths[ind]).raw_image.astype(np.float32)
            except:
                raw = np.fromfile(input_paths[ind], dtype=np.uint16)
                raw = raw.reshape(2848, 4256)
                input_raw = raw.astype(np.float32)
