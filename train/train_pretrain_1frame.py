...
from models import RViDeNet, RViDeNet_sfb, RViDeNet_ECBAM, RViDeNet_, Predenoiser
from utils import *
...
save_dir ="/database/iyj0121/dataset/4k_vnt/ckpt/pretrain/pretrain_noise_estimation_crvd_method_neif_on_4k_calibration_gain_6_30_hcg_unet"
...
#denoiser = RViDeNet_DCNv4(predenoiser=predenoiser).cuda()
##################################################################################################################################################################
...
for epoch in range(initial_epoch, args.num_epochs+1):
    ...
    for batch_id in range(int(len(gt_paths)/batch_size)):
        ...
        while batch_num<batch_size:
            ...
            gt_frame_name = generate_name(frame_id)
            ...
            gt_patch = gt_raw[yy:yy + ps * 2, xx:xx + ps * 2]
            ...
            input_batch_list.append(input_pack)
        ...
        denoised_out = denoiser(in_data.reshape(1,4,ps,ps))
